	...
	var des = {
		 ...
		 * string
		 */
		strEnc : function(data, firstKey, secondKey, thirdKey) {
			...
			if (leng > 0) {
				if (leng < 4) {
					...
					if (remainder > 0) {
						var remainderData = data.substring(iterator * 4 + 0,
								leng);
		...
		enc : function(dataByte, keyByte) {
			...
			for (i = 0; i < 16; i++) {
				...
				var tempRight = this.xor(this.pPermute(this.sBoxPermute(this.xor(
						this.expandPermute(ipRight), key))), tempLeft);
				for (n = 0; n < 32; n++) {
	 ...
	 * function test() {
	 * 
	 * var msg = "abcdefgh"; var bt = this.strToBt(msg);
	 * 
	 * var key = "12345678"; var keyB = this.strToBt(key);
	 * 
	 * var encByte = this.enc(bt,keyB);
	 * 
	 * var enchex = this.bt64ToHex(encByte); endata.value=enchex;
	 * 
	 * var encStr = this.hexToBt64(enchex); alert("encStr="+encStr); var eByte =
	 * new Array(); for(m=0;m<encStr.length;m++){ eByte[m] =
	 * parseInt(encStr.substring(m,m+1)); } var decbyte= this.dec(eByte,keyB)
	 * var decmsg= this.byteToString(decbyte); alert("decbyte="+decbyte);
	 * alert("decmsg="+decmsg); }
	 * 
	 * des.strEnc($("#tbPassword").val(), _csrf)
	 */